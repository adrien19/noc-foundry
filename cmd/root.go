...
import (
	...
	"github.com/adrien19/noc-foundry/cmd/internal/invoke"
	"github.com/adrien19/noc-foundry/cmd/internal/skills"
...
func GenerateCommand(out, err io.Writer) *cobra.Command {
	opts := internal.NewNOCFoundryOptions(internal.WithIOStreams(out, err))
...
func Execute() {
	// Initialize options
	opts := internal.NewNOCFoundryOptions()
...
func NewCommand(opts *internal.NOCFoundryOptions) *cobra.Command {
	...
	flags.BoolVar(&opts.Cfg.UI, "ui", false, "Launches the NOCFoundry UI web server.")
...
func handleDynamicReload(ctx context.Context, toolsFile internal.ToolsFile, s *server.Server, authCfg server.ServerAuthConfig) error {
	logger, err := util.LoggerFromContext(ctx)
	if err != nil {
		panic(err)
	}

	sourcesMap, authServicesMap, embeddingModelsMap, toolsMap, toolsetsMap, promptsMap, promptsetsMap, err := validateReloadEdits(ctx, toolsFile, authCfg)
	if err != nil {
...
func validateReloadEdits(
	ctx context.Context, toolsFile internal.ToolsFile, authCfg server.ServerAuthConfig,
	...
	ctx, span := instrumentation.Tracer.Start(ctx, "nocfoundry/server/reload")
	defer span.End()
	...
	reloadedConfig := server.ServerConfig{
		...
		DeviceGroupConfigs:    toolsFile.DeviceGroups,
	}
	...
	if err := server.ValidateAndApplyEndpointAuthConfig(authCfg, authServicesMap); err != nil {
		errMsg := fmt.Errorf("unable to validate reloaded server auth config: %w", err)
		logger.WarnContext(ctx, errMsg.Error())
		return nil, nil, nil, nil, nil, nil, nil, err
	}
...
func watchChanges(ctx context.Context, watchDirs map[string]bool, watchedFiles map[string]bool, s *server.Server, pollTickerSecond int, authCfg server.ServerAuthConfig) {
	logger, err := util.LoggerFromContext(ctx)
	if err != nil {
		panic(err)
	}
	...
	for {
		...
		case <-debounce.C:
			...
			err = handleDynamicReload(ctx, reloadedToolsFile, s, authCfg)
			if err != nil {
...
func run(cmd *cobra.Command, opts *internal.NOCFoundryOptions) error {
	...
	if err != nil {
		errMsg := fmt.Errorf("nocfoundry failed to initialize: %w", err)
	...
	if opts.Cfg.Stdio {
		...
		err = s.Listen(ctx)
		if err != nil {
			errMsg := fmt.Errorf("nocfoundry failed to start listener: %w", err)
			opts.Logger.ErrorContext(ctx, errMsg.Error())
			return errMsg
		}
		opts.Logger.InfoContext(ctx, "Server ready to serve!")
		if opts.Cfg.UI {
			opts.Logger.InfoContext(ctx, fmt.Sprintf("NOCFoundry UI is up and running at: http://%s:%d/ui", opts.Cfg.Address, opts.Cfg.Port))
		}
	...
	if isCustomConfigured && !opts.Cfg.DisableReload {
		...
		go watchChanges(ctx, watchDirs, watchedFiles, s, opts.Cfg.PollInterval, opts.Cfg.Auth)
	}
	...
	case err := <-srvErr:
		if err != nil {
			errMsg := fmt.Errorf("nocfoundry crashed with the following error: %w", err)
			opts.Logger.ErrorContext(ctx, errMsg.Error())
			return errMsg
		}
	case <-ctx.Done():
