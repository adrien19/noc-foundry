...
import (
	...
	"github.com/adrien19/noc-foundry/internal/sources/http"
...
func TestInitializeRejectsNonPositiveTimeout(t *testing.T) {
	ctx, err := testutils.ContextWithNewLogger()
	if err != nil {
		t.Fatalf("unable to create context logger: %v", err)
	}
	...
	if err == nil {
		t.Fatalf("expected Initialize to fail for non-positive timeout")
	}
...
func TestInitializeRequiresBaseURL(t *testing.T) {
	...
	_, err = cfg.Initialize(ctx, nil)
	if err == nil {
		t.Fatalf("expected Initialize to fail for missing baseUrl")
	}
