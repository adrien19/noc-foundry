...
// Package showalarms implements the network-show-alarms MCP tool.
// This tool retrieves active system alarms from any device with a registered profile.
...
const kind = "network-show-alarms"
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	if desc == "" {
		desc = "Show active system alarms on a network device. Returns canonical alarm data via the best available protocol (gNMI, NETCONF, or CLI)."
	}
...
func (t Tool) Invoke(ctx context.Context, resourceMgr tools.SourceProvider, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.NOCFoundryError) {
	return profilequery.Invoke(ctx, resourceMgr, t.executor, t.Source, t.SourceSelector, profiles.OpGetSystemAlarms, params)
}
