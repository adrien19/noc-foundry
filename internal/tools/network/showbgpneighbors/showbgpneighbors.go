...
// Package showbgpneighbors implements the network-show-bgp-neighbors MCP tool.
// This tool retrieves BGP neighbor state from any device with a registered profile.
...
const kind = "network-show-bgp-neighbors"
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	if desc == "" {
		desc = "Show BGP neighbor state on a network device. Returns canonical BGP neighbor data via the best available protocol (gNMI, NETCONF, or CLI)."
	}
...
func (t Tool) Invoke(ctx context.Context, resourceMgr tools.SourceProvider, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.NOCFoundryError) {
	return profilequery.Invoke(ctx, resourceMgr, t.executor, t.Source, t.SourceSelector, profiles.OpGetBGPNeighbors, params)
}
