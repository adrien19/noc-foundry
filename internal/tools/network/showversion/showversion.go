...
// Package showversion implements the network-show-version MCP tool,
// a vendor-agnostic tool that retrieves system version information from
// any device that has a registered profile.
package showversion
...
const kind = "network-show-version"
...
// Config holds the YAML-decoded configuration for the network show version tool.
type Config struct {
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	if desc == "" {
		if cfg.SourceSelector != nil {
			desc = "Show system version information on network devices matching a label selector. Pass 'device' to target a specific device, or omit to query all matching devices."
		} else {
			desc = "Show system version information on a network device. Returns hostname, software version, and chassis type via the best available protocol (gNMI, NETCONF, or CLI)."
		}
	}
...
// Tool implements the vendor-agnostic show version operation.
type Tool struct {
	Config
