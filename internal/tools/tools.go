...
type Tool interface {
	Invoke(context.Context, SourceProvider, parameters.ParamValues, AccessToken) (any, util.NOCFoundryError)
...
type SourceProvider interface {
	...
	// GetSourcesByLabels returns sources matching the given label criteria.
	// All specified labels must match (AND semantics).
	// Returns nil, nil if no device pool is configured.
	GetSourcesByLabels(ctx context.Context, matchLabels map[string]string) (map[string]sources.Source, error)
	// GetDevicePoolLabels returns the label index for all device-pool sources,
	// or nil if no device pool is configured.
	GetDevicePoolLabels() map[string]map[string]string
}
...
func GetMcpManifest(name, desc string, authInvoke []string, params parameters.Parameters, annotations *ToolAnnotations) McpManifest {
	...
	if len(authInvoke) > 0 {
		metadata["nocfoundry/authInvoke"] = authInvoke
	}
	if len(authParams) > 0 {
		metadata["nocfoundry/authParam"] = authParams
	}
	if len(metadata) > 0 {
