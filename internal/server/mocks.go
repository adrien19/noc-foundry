...
func (t MockTool) Invoke(context.Context, tools.SourceProvider, parameters.ParamValues, tools.AccessToken) (any, util.NOCFoundryError) {
	mock := []any{t.Name}
	return mock, nil
}

func (t MockTool) CompileValidationRun(context.Context, tools.SourceProvider, parameters.ParamValues) (validation.CompiledRun, error) {
	return validation.CompiledRun{}, fmt.Errorf("not implemented")
}

func (t MockTool) ExecuteCompiledValidation(context.Context, tools.SourceProvider, validation.CompiledRun, tools.AccessToken) (any, util.NOCFoundryError) {
	return nil, util.NewClientServerError("not implemented", 500, nil)
}
...
func (t MockTool) McpManifest() tools.McpManifest {
	...
	if len(authParams) > 0 {
		mcpManifest.Metadata = map[string]any{
			"nocfoundry/authParams": authParams,
		}
	}
